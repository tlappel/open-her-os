...
If that fails, install directly: `npx claude-mem install --ide claude-code` (or `--ide copilot-cli` for Copilot). This registers the claude-mem plugin, which provides the MCP server.
